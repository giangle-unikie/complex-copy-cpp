...
TEST(pipeSendTest7, tranferFile){
    ...
    int argc = 5;
    char* argv1[] = {mode, pipeMethod, methodName, file, sendFileName};
    ipcHandler ipc;
    ipc_info info;
    ...
    if(info.mode == IPCMode::SEND_MODE && info.protocol == IPCProtocol::PIPE){
        c = boost::process::child("testFile/ipc_receive", "-p", "methodName", "-f", "fileName");
    }
